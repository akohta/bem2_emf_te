...
void dat_read (char *fname,DOMD *md)
{
  ...
  if(fread(md->med_fn,sizeof(int),128,fp)!=128){
    printf("d2te_setup.c, dat_read(), failed to read the med_fn. exit...\n");
    exit(1);
  }
  if(fread(md->msh_fn,sizeof(int),128,fp)!=128){
    printf("d2te_setup.c, dat_read(), failed to read the msh_fn. exit...\n");
    exit(1);
  }
  // rotation and translation data
  if(fread(&(md->ra),sizeof(double),1,fp)!=1){
    printf("d2te_setup.c, dat_read(), failed to read the ra. exit...\n");
    exit(1);
  }
  if(fread(&(md->tx),sizeof(double),1,fp)!=1){
    printf("d2te_setup.c, dat_read(), failed to read the tx. exit...\n");
    exit(1);
  }
  if(fread(&(md->ty),sizeof(double),1,fp)!=1){
    printf("d2te_setup.c, dat_read(), failed to read the ty. exit...\n");
    exit(1);
  }
  // material def
  if(fread(&(md->MN),sizeof(int),1,fp)!=1){
    printf("d2te_setup.c, dat_read(), failed to read the MN. exit...\n");
    exit(1);
  }
  md->n=(double complex *)malloc(sizeof(double complex)*(md->MN+1)); // malloc
  if(fread(md->n,sizeof(double complex),md->MN+1,fp)!=md->MN+1){
    printf("d2te_setup.c, dat_read(), failed to read the n. exit...\n");
    exit(1);
  }
  // incident field def
  if(fread(&(md->wd),sizeof(INFD),1,fp)!=1){
    printf("d2te_setup.c, dat_read(), failed to read the wd. exit...\n");
    exit(1);
  }
  // boundary def
  if(fread(&(md->bd.Nn),sizeof(int),1,fp)!=1){
    printf("d2te_setup.c, dat_read(), failed to read the Nn. exit...\n");
    exit(1);
  }
  if(fread(&(md->bd.Ne),sizeof(int),1,fp)!=1){
    printf("d2te_setup.c, dat_read(), failed to read the Ne. exit...\n");
    exit(1);
  }
  malloc_node(&(md->bd));  malloc_elem(&(md->bd)); // malloc
  if(fread(md->bd.et,sizeof(double),7,fp)!=7){
    printf("d2te_setup.c, dat_read(), failed to read the et. exit...\n");
    exit(1);
  }
  if(fread(md->bd.wg,sizeof(double),7,fp)!=7){
    printf("d2te_setup.c, dat_read(), failed to read the wg. exit...\n");
    exit(1);
  }
  if(fread(md->bd.wk,sizeof(double),7,fp)!=7){
    printf("d2te_setup.c, dat_read(), failed to read the wk. exit...\n");
    exit(1);
  }
  if(fread(md->bd.M,sizeof(double),21,fp)!=21){
    printf("d2te_setup.c, dat_read(), failed to read the M. exit...\n");
    exit(1);
  }
  if(fread(md->bd.xn,sizeof(double),md->bd.Nn+1,fp)!=md->bd.Nn+1){
    printf("d2te_setup.c, dat_read(), failed to read the xn. exit...\n");
    exit(1);
  }  
  ...
  if(fread(md->bd.xh,sizeof(double),GLH,fp)!=GLH){
    printf("d2te_setup.c, dat_read(), failed to read the GLH. exit...\n");
    exit(1);
  }  
  ...
  for(i=0;i<=md->bd.Ne;i++){
    if(fread(md->bd.ed[i],sizeof(int),3,fp)!=3){
      printf("d2te_setup.c, dat_read(), failed to read the ed[i]. exit...\n");
      exit(1);
    }
  }
  for(i=0;i<=md->bd.Ne;i++){
    if(fread(md->bd.x[i],sizeof(double),7,fp)!=7){
      printf("d2te_setup.c, dat_read(), failed to read the x[i]. exit...\n");
      exit(1);
    }
  }
  for(i=0;i<=md->bd.Ne;i++){
    if(fread(md->bd.y[i],sizeof(double),7,fp)!=7){
      printf("d2te_setup.c, dat_read(), failed to read the y[i]. exit...\n");
      exit(1);
    }
  }
  for(i=0;i<=md->bd.Ne;i++){
    if(fread(md->bd.dx[i],sizeof(double),7,fp)!=7){
      printf("d2te_setup.c, dat_read(), failed to read the dx[i]. exit...\n");
      exit(1);
    }
  }
  for(i=0;i<=md->bd.Ne;i++){
    if(fread(md->bd.dy[i],sizeof(double),7,fp)!=7){
      printf("d2te_setup.c, dat_read(), failed to read the dy[i]. exit...\n");
      exit(1);
    }
  }
  for(i=0;i<=md->bd.Ne;i++){
    if(fread(md->bd.ui[i],sizeof(double complex),3,fp)!=3){
      printf("d2te_setup.c, dat_read(), failed to read the ui[i]. exit...\n");
      exit(1);
    }
  }
  for(i=0;i<=md->bd.Ne;i++){
    if(fread(md->bd.duidn[i],sizeof(double complex),3,fp)!=3){
      printf("d2te_setup.c, dat_read(), failed to read the duidn[i]. exit...\n");
      exit(1);
    }
  }
  // sub domain def
  ...
  for(i=0;i<=md->MN;i++){
    if(fread(md->bd.sb[i].eid,sizeof(int),md->bd.sb[i].Ne+1,fp)!=md->bd.sb[i].Ne+1){
      printf("d2te_setup.c, dat_read(), failed to read the eid. exit...\n");
      exit(1);
    }
    for(j=0;j<=md->bd.sb[i].Ne;j++){
      if(fread(md->bd.sb[i].u[j],sizeof(double complex),3,fp)!=3){
        printf("d2te_setup.c, dat_read(), failed to read the u[j]. exit...\n");
        exit(1);
      }
      if(fread(md->bd.sb[i].v[j],sizeof(double complex),3,fp)!=3){
        printf("d2te_setup.c, dat_read(), failed to read the v[j]. exit...\n");
        exit(1);
      }
      if(fread(md->bd.sb[i].w[j],sizeof(double complex),3,fp)!=3){
        printf("d2te_setup.c, dat_read(), failed to read the w[j]. exit...\n");
        exit(1);
      }
      if(fread(md->bd.sb[i].dudn[j],sizeof(double complex),3,fp)!=3){
        printf("d2te_setup.c, dat_read(), failed to read the dudn[j]. exit...\n");
        exit(1);
      }
      if(fread(md->bd.sb[i].dvdn[j],sizeof(double complex),3,fp)!=3){
        printf("d2te_setup.c, dat_read(), failed to read the dvdn[j]. exit...\n");
        exit(1);
      }
      if(fread(md->bd.sb[i].dwdn[j],sizeof(double complex),3,fp)!=3){
        printf("d2te_setup.c, dat_read(), failed to read the dwdn[j]. exit...\n");
        exit(1);
      }
    }
  }
...
void read_medium_data(char *med_fn,DOMD *md)
{
  ...
  if(fgets(buf,256,fp)==NULL){
    printf("d2te_setup.c, read_medium_data(), failed to read the line. exit...\n");
    exit(1);
  }
  if(fgets(buf,256,fp)==NULL){
    printf("d2te_setup.c, read_medium_data(), failed to read the line. exit...\n");
  ...
  if(fscanf(fp,"%d\n",&ti)!=1){
    printf("d2te_setup.c, read_medium_data(), failed to read the MN. exit...\n");
    exit(1);
  }
  md->MN=ti;
  ...
  for(i=1;i<=md->MN;i++){
    if(fscanf(fp,"%lf",&td)!=1){
      printf("d2te_setup.c, read_medium_data(), failed to read the real(n). exit...\n");
      exit(1);
    }
    if(fscanf(fp,"%lf",&td2)!=1){
      printf("d2te_setup.c, read_medium_data(), failed to read the imag(n). exit...\n");
      exit(1);
    }
    md->n[i]=td+td2*I;
  }
