...
void solve_bieq(DOMD *md)
{
  ...
  printf("  solve boundary value Ez     "); fflush(stdout);
  solve_mkl_lapack(&cm);
  calc_bv_u_dudn(&cm,md);
  printf(" finished\n");

  printf("  boundary value Hx Hy        "); fflush(stdout);
  calc_bv_vw(md);
  printf(" finished\n");
...
void create_matrix_domain(int did,CMD *cm,DOMD *md)
{
  ...
  FILE *fg,*fh;
  double complex *tG,*tH,CC[7],k,ctp;
  double rt[2],F;
  ...
  for(t=1;t<=Ne;t++){
    ...
    for(j=0;j<3;j++){
      ...
      F=0.0;
      #pragma omp parallel for schedule(dynamic) reduction(+:F) private(sd,asd,CC,i,ctp)
      for(s=1;s<=Ne;s++){
        ...
        if(did==md->bd.md[asd]){
          ...
          for(i=0;i<3;i++){
            ...
            cm->A[cm->tt*cm->N+cm->N/2*0+(asd-1)*3+i].imag=cimag( CC[3+i]);
            cm->A[cm->tt*cm->N+cm->N/2*1+(asd-1)*3+i].real=creal( CC[  i]);
            cm->A[cm->tt*cm->N+cm->N/2*1+(asd-1)*3+i].imag=cimag( CC[  i]);
            if(md->bd.md[asd]==0){
              ctp=-CC[3+i]*md->bd.ui[asd][i]-CC[i]*md->bd.duidn[asd][i];
              #pragma omp atomic
    ...
    for(s=1;s<=md->bd.sb[d].Ne;s++){
      ...
      if(md->bd.md[aeid]==d){
        ...
        if(md->bd.md[aeid]!=0){
          ...
          md->bd.sb[d].u[s][2]=cm->B[(aeid-1)*3+2].real+cm->B[(aeid-1)*3+2].imag*I;
          md->bd.sb[d].dudn[s][0]=-cm->B[cm->N/2+(aeid-1)*3+1].real-cm->B[cm->N/2+(aeid-1)*3+1].imag*I;
          md->bd.sb[d].dudn[s][1]=-cm->B[cm->N/2+(aeid-1)*3+0].real-cm->B[cm->N/2+(aeid-1)*3+0].imag*I;
          md->bd.sb[d].dudn[s][2]=-cm->B[cm->N/2+(aeid-1)*3+2].real-cm->B[cm->N/2+(aeid-1)*3+2].imag*I;
        }
        else {
          ...
          md->bd.sb[d].dudn[s][0]=-cm->B[cm->N/2+(aeid-1)*3+1].real-cm->B[cm->N/2+(aeid-1)*3+1].imag*I-md->bd.duidn[aeid][1];
          md->bd.sb[d].dudn[s][1]=-cm->B[cm->N/2+(aeid-1)*3+0].real-cm->B[cm->N/2+(aeid-1)*3+0].imag*I-md->bd.duidn[aeid][0];
          md->bd.sb[d].dudn[s][2]=-cm->B[cm->N/2+(aeid-1)*3+2].real-cm->B[cm->N/2+(aeid-1)*3+2].imag*I-md->bd.duidn[aeid][2];
        }
      }
    }
  }
  ...
  for(d=0;d<=md->MN;d++){
    ...
    for(s=1;s<=md->bd.sb[d].Ne;s++){
      ...
      for(i=0;i<3;i++){
        ...
        if      (SFDM==0) dudt=dEzdt_bv_node_dbieq(d,s,i,md);
        else if (SFDM==1) dudt=dEzdt_bv_node_ndmtd(d,s,i,md);
        else {
          printf("SFDM parameter error. SFDM=%d. check pb_const.h Exit...\n",SFDM);
          exit(1);
        }
        md->bd.sb[d].v[s][i]= i_k2*i_aJ*(-md->bd.sb[d].dudn[s][i]*dx+dudt*dy);
        md->bd.sb[d].w[s][i]=-i_k2*i_aJ*( md->bd.sb[d].dudn[s][i]*dy+dudt*dx);
      }
    }
  }
}
...
void create_amatrix_subdomain(int did,CMD *cm,DOMD *md)
{
  ...
  for(j=0;j<cm->N;j++){
    if(fread(tG,sizeof(double complex),cm->N,fg)!=cm->N){
      printf("d2te_solve_bieq.c, create_amatrix_subdomain(), failed to read the tH. exit...\n");
      exit(1);
    }
    for(s=0;s<cm->N;s++){
...
void create_bmatrix_subdomain(int did,CMD *cm,double complex **u)
{
  ...
  for(j=0;j<cm->N;j++){
    ...
    if(fread(tH,sizeof(double complex),cm->N,fh)!=cm->N){
      printf("d2te_solve_bieq.c, create_bmatrix_subdomain(), failed to read the tH. exit...\n");
      exit(1);
    }
    for(s=1;s<=cm->N/3;s++){
      for(sn=0;sn<3;sn++){
