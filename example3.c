...
typedef struct image_data{
  ...
  double mez,mhr[2];      // maximum amplitude of each field component
}IMD;
...
int main(int argc,char *argv[])
{
  ...
  id.ve=(double complex *)m_alloc2(id.m*id.m*1,sizeof(double complex),"example3.c, ve");
  id.vh=(double complex *)m_alloc2(id.m*id.m*2,sizeof(double complex),"example3.c, vh");
...
void eh_field_z(IMD *id,DOMD *md)
{
  ...
  // z=0 plane  
  #pragma omp parallel for schedule(dynamic) private(x,j,ez,hr) 
  for(i=0;i<id->m;i++){
    x[1]=id->rang-(double)i*dr;
    for(j=0;j<id->m;j++){
      x[0]=-id->rang+(double)j*dr;
      EH_t(&ez,hr,x,id->type,md); // total field
      
      #pragma omp critical
      {
        if(cabs(ez)>id->mez) id->mez=cabs(ez);
        if(cabs(hr[0])>id->mhr[0]) id->mhr[0]=cabs(hr[0]);
        if(cabs(hr[1])>id->mhr[1]) id->mhr[1]=cabs(hr[1]);
      }
      
      id->ve[i*id->m+j]=ez;
      id->vh[i*id->m*2+j*2+0]=hr[0];
      id->vh[i*id->m*2+j*2+1]=hr[1];
    }
  }
}
...
void output_png(int nt,double complex cet,char *pl,IMD *id)
{
  ...
  for(i=0;i<m;i++){
    for(j=0;j<m;j++){
      color_rgb(creal(cet*id->ve[i*m+j])/id->mez,&r,&g,&b);
      ...
      color_rgb(creal(cet*id->vh[i*m*2+j*2+0])/id->mhr[0],&r,&g,&b);
      for(si=0;si<=scale;si++){
        for(sj=0;sj<=scale;sj++){
      ...
      color_rgb(creal(cet*id->vh[i*m*2+j*2+1])/id->mhr[1],&r,&g,&b);
      for(si=0;si<=scale;si++){
        for(sj=0;sj<=scale;sj++){
